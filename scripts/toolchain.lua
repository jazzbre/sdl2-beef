...
function toolchain(_buildDir, _libDir)
	...
	local androidPlatformVersion = "23"
	local androidPlatform = "android-" .. androidPlatformVersion

	local ANDROID_NDK_LLVM = os.getenv("ANDROID_NDK_LLVM")
	local ANDROID_NDK_ARM = os.getenv("ANDROID_NDK_ARM")
	local ANDROID_NDK_ARM64 = os.getenv("ANDROID_NDK_ARM64")
	local ANDROID_NDK_MIPS = os.getenv("ANDROID_NDK_MIPS")
	local ANDROID_NDK_X86 = os.getenv("ANDROID_NDK_X86")
	local ANDROID_NDK_ROOT = os.getenv("ANDROID_NDK_ROOT")

	if not ANDROID_NDK_LLVM then
		ANDROID_NDK_LLVM = ""
	end

	if not ANDROID_NDK_ARM then
		ANDROID_NDK_ARM = ""
	end

	if not ANDROID_NDK_ARM64 then
		ANDROID_NDK_ARM64 = ""
	end

	if not ANDROID_NDK_MIPS then
		ANDROID_NDK_MIPS = ""
	end

	if not ANDROID_NDK_X86 then
		ANDROID_NDK_X86 = ""
	end

	if not ANDROID_NDK_ROOT then
		ANDROID_NDK_ROOT = ""
	end
	...
	if _ACTION == "gmake" or _ACTION == "ninja" then
		...
		if "android-arm" == _OPTIONS["gcc"] then
			...
			or not os.getenv("ANDROID_NDK_LLVM") then
				print("Set ANDROID_NDK_CLANG, ANDROID_NDK_ARM, and ANDROID_NDK_LLVM environment variables.")
			end

			premake.gcc.cc  = quote(ANDROID_NDK_LLVM .. "/bin/clang")
			premake.gcc.cxx = quote(ANDROID_NDK_LLVM .. "/bin/clang++")
			premake.gcc.ar  = quote(ANDROID_NDK_LLVM .. "/bin/arm-linux-androideabi-ar")
		...
		elseif "android-arm64" == _OPTIONS["gcc"] then
			...
			or not os.getenv("ANDROID_NDK_CLANG")
			or not os.getenv("ANDROID_NDK_LLVM") then
				print("Set ANDROID_NDK_CLANG, ANDROID_NDK_ARM64, and ANDROID_NDK_LLVM environment variables.")
			end

			premake.gcc.cc  = quote(ANDROID_NDK_LLVM .. "/bin/clang")
			premake.gcc.cxx = quote(ANDROID_NDK_LLVM .. "/bin/clang++")
			premake.gcc.ar  = quote(ANDROID_NDK_LLVM .. "/bin/aarch64-linux-android-ar")
		...
		elseif "android-x86" == _OPTIONS["gcc"] then
			...
			or not os.getenv("ANDROID_NDK_CLANG")
			or not os.getenv("ANDROID_NDK_LLVM") then
				print("Set ANDROID_NDK_CLANG, ANDROID_NDK_X86, and ANDROID_NDK_LLVM environment variables.")
			end

			premake.gcc.cc  = quote(ANDROID_NDK_LLVM .. "/bin/clang")
			premake.gcc.cxx = quote(ANDROID_NDK_LLVM .. "/bin/clang++")
			premake.gcc.ar  = quote(ANDROID_NDK_LLVM .. "/bin/i686-linux-android-ar")
			premake.gcc.llvm = true
...
--			"-Wuseless-cast",
	...
	configuration { "android-*" }
		...
		includedirs {
			...
			ANDROID_NDK_LLVM .. "/sysroot/usr/include",
   ...
   configuration { "android-arm or android-x86" }
      links {
         "android_support",
      }
	...
	configuration { "android-arm" }
		...
		libdirs {
			...
			quote(ANDROID_NDK_LLVM .. "sysroot/usr/lib/arm-linux-androideabi"),
		}
		includedirs {
			quote(ANDROID_NDK_LLVM .. "sysroot/usr/include/arm-linux-androideabi"),
		}
		buildoptions {
			"-target armv7-none-linux-androideabi" .. androidPlatformVersion,
			"-mthumb",
			"-march=armv7-a",
			"-mfloat-abi=softfp",
			"-mfpu=neon",
	...
	configuration { "android-arm64" }
		...
		libdirs {
			path.join(_libDir, "lib/android-arm64"),
			quote(ANDROID_NDK_LLVM .. "sysroot/usr/lib/aarch64-linux-android"),
		}
		includedirs {
			quote(ANDROID_NDK_LLVM .. "sysroot/usr/include/aarch64-linux-android"),
		}
		buildoptions {
			"-target aarch64-none-linux-android" .. androidPlatformVersion,
			"-march=armv8-a",
			"-Wunused-value",
			"-Wundef",
		}
	...
	configuration { "android-x86" }
		...
		libdirs {
			...
			quote(ANDROID_NDK_LLVM .. "sysroot/usr/lib/i686-linux-android"),
		}
		includedirs {
			quote(ANDROID_NDK_LLVM .. "sysroot/usr/include/i686-linux-android"),
		}
		buildoptions {
			"-target i686-none-linux-android" .. androidPlatformVersion,
			"-march=i686",
			"-mtune=atom",
			"-mstackrealign",
			"-msse3",
